...
{ 
    ...
    //printf("b: %s\n",b);
    static char name[20]={""};
    strncpy(name, a, strlen(a)-strlen(b));//注意后面的6，如果你的测试集的图片的名字字符（不包括后缀）是其他长度，请改为你需要的长度（官方的默认的长度是6）
    return name;
}
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, float thresh, char *outfile)
{
    ...
    char *valid_images = option_find_str(options, "valid", "data/train.list");
    char *name_list = option_find_str(options, "names", "data/names.list");
    /* options中的内容某一行的等号左边如果是"results"， 则返回该行， 否则返回字符串"results" */
    char *prefix = option_find_str(options, "results", "results");
    char **names = get_labels(name_list);
    char *mapf = option_find_str(options, "map", 0);
    int *map = 0;
    ...
    char buff[1024];
    /* options中的内容某一行的等号左边如果是"eval"， 则返回该行， 否则返回字符串"others" */
    char *type = option_find_str(options, "eval", "others");
    printf("type %s\n", type);
...
void validate_detector_recall(char *datacfg, char *cfgfile, char *weightfile)
{
    ...
    /* Modified by kxlong, 2019.4.3  */
    list *options = read_data_cfg(datacfg);
    /* options中的内容某一行的等号左边如果是"valid"， 则返回该行， 否则返回字符串"data/train.list" */
    char *valid_images = option_find_str(options, "valid", "data/train.list");
    printf("%s\n", valid_images);
    list *plist = get_paths(valid_images);
    char **paths = (char **)list_to_array(plist);
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    list *options = read_data_cfg(datacfg);
    /* options->size, 
     options->front->val->val, options->front->val->key, options->front->val->used,
     options->back->val->val, options->back->val->key, options->back->val->used */

    /* options->front->val->val是datacfg的文件中的每行的内容， options->back->val->key是datacfg的每行内容的等号左边的字符串, 
    如果 等号左边的字符串=="names"，则返回datacfg的等号左边为"names"的行, 否则返回"data/names.list" */
            ...
            if(imgNum > 1)
            {
            	...
            	for(i = 0;i < imgNum;i++)
            	{
			        ...
			        if(outfile){
			            ...
			            char detectImg[256] = {0};
			            sprintf(detectImg,"./%s%s%s", "results/", "detected",GetFilename(path));
			            printf("%s\n", detectImg);
			            save_image(im, detectImg);
...
{
	/* prefix即为"results" */
	/* thresh是输出第五列，confidence表示是前景的概率 */
	/* hier_thresh是分层分类的一个参数， ImageNet的标签是来自WordNet的。WordNet的结构是一个有向图而不是树（因为树中每个节点只有一个双亲）。作者将问题简化成从ImageNet中构建一个分层的树。*/
    char *prefix = find_char_arg(argc, argv, "-prefix", 0);
    float thresh = find_float_arg(argc, argv, "-thresh", .5);
    float hier_thresh = find_float_arg(argc, argv, "-hier", .5);
    /* 2019.4.26 by kxlong */
    float nms = find_float_arg(argc, argv, "-num", .45);
    /**/
    ...
    if(0==strcmp(argv[2], "test")) 
        ...
        validate_detector(datacfg, cfg, weights, thresh, outfile);
