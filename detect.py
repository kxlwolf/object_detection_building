...
#        prediction = prediction[:,scale_indices]
        ...
        print("prediction shapeA: ", prediction.shape)
...
#        print(end - start)
        ...
        for im_num, image in enumerate(imlist[i*batch_size: min((i +  1)*batch_size, len(imlist))]):
            print("!!!!!!!!!!!!!!!!!!!!!")
            print("im_num: ",im_num,  "image: ", image)
            im_id = i*batch_size + im_num
            print("im_id: ", im_id)
            """
            objs列表包含了本次处理图片中所有检测得到的方框所包含目标的类别名称。每个元素对应一个检测得到的方框所包含目标的类别名称。output遍历output中的每一行(即一个方框的预测值)得到x，如果这个方
